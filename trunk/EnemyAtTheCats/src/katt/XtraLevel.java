...
public class XtraLevel extends TheGame {
	...
	public void init(GameContainer container, StateBasedGame game)
		...
		// Moving background 1
		backgrounds[0] = new Image("data/Img/black.png");
		backgrounds[1] = new Image("data/Img/black.png");

		// Moving background 2
		backgrounds[2] = new Image("data/Img/Space1.png");
		backgrounds[3] = new Image("data/Img/Space2.png");

		backgrounds[4] = new Image("data/Img/Planets1.png");
	...
	private void generateEnemys() {
		...
		int numberOfEnemys = getCurrentLevel(); // Minimum number of Enemys
												// equal to level
		int numberOfExtraEnemys = randomGenerator.nextInt(getCurrentLevel()); // Extra
																				// Enemys
		int totalEnemys = numberOfEnemys + numberOfExtraEnemys; // Total number
																// of Enemys to
																// add

		for (int i = 0; i < totalEnemys; i++) {
			fEnemys1.add(new FlyingEnemy(randomGenerator.nextInt(13))); // Add
																		// random
																		// type
																		// of
																		// Enemy
	...
	public void update(GameContainer container, StateBasedGame game, int delta) {
		...
		for (int x = 2; x < backgroundPos.length; x++) {
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g) {
		...
		if (game.getState(StateHandler.XTRALEVEL) == game.getCurrentState()) {
			for (Player1 pl : players) {
				...
				((ConfigurableEmitter) rocketFire.getEmitter(0)).setPosition(
						(pl.getPlayerX() + 15), pl.getPlayerY() + 35);
				rocketFire.render();
			...
			// Draw Enemy-objects
			for (FlyingEnemy fEnemy : fEnemys1) {
				g.drawImage(fEnemy.getfEnemyImage(), fEnemy.getPosX(),
						fEnemy.getPosY());
			}
