...
public class Connect {
	...
	// Metod för att connecta mot en mysql databas. Returns conn som man kan
	// använda för att ställa frågor i utomstående metoder.
	public static Connection connect() {
		...
		try {
			conn = DriverManager.getConnection(url, "scrum", "stiga123");// inlogg
																			// och
																			// lösen
		...
		try {
			// gör ett statement och skickar iväg. i det här fallet en update.
			// execute update kan även innehålla insert och delete.
			Statement stmt = conn.createStatement();
			stmt.executeUpdate("UPDATE highscore SET telefon='0701231234' WHERE namn='Yahoo'");

			// fråga om man vill få ut ett resultat. läggs med fördel i en
			// resultset.
